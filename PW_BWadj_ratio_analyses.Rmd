...
ggsave(filename = here::here("Mariana", "Env Int Revision", "output", "BW_PW_ratio_spline_figure.pdf"),
...
ggsave(filename = here::here("Mariana", "Env Int Revision", "output", "BW_PW_ratio_spline_figure_N253_N251.pdf"),
...
ggsave(filename = here::here("Mariana", "Env Int Revision", "output", "supplementary_sensitivity_analysis_P_distributions.pdf"),
...
There are 9 genes associated with a 1-SD change in BW:PW ratio in covariate adjusted models (FDR<0.05).

```{r}
Results_ratio |>
  dplyr::as_tibble(rownames = pkgconfig::get_config("tibble::rownames", "ensembl_gene_id")) |>
  dplyr::filter(adj.P.Val<0.05) |>
  dplyr::relocate(genes, logFC) |>
...
mod_ratio_sex <- with(covar_ratio, 
                model.matrix(~ h_c_sex/ratio_scale + 
                               labor_collapsed + 
                               h_del_method + 
                               batch +
                               h_m_delivery_age + 
...
hist(Results_bw_male$P.Value, breaks = 20, main = "Birthweight adjusted: Males")
abline(h = 13072/20)
...
hist(Results_bw_female$P.Value, breaks = 20, main = "Birthweight adjusted: Females")
abline(h = 13072/20)
...
overall_volcano_plots <- volcano_data |> 
...
overall_volcano_plots

ggsave(filename = here::here("Mariana", "Env Int Revision", "output", "BWadj_Ratio_overall_volcano_plot.pdf"),
...
ggsave(filename = here::here("Mariana", "Env Int Revision", "output", "BWadj_Ratio_sex_specific_volcano_plot.pdf"),
...
ggsave(filename = here::here("Mariana", "Env Int Revision", "output", "birthweight_ratio_concordance_logFC.pdf"),
...
overlapping_genes_plot <- dplyr::bind_rows(
  Results_bw |> dplyr::mutate(analysis = "BW~adj~"),
  Results_ratio |> dplyr::mutate(analysis = "BW:PW")
) |>
  dplyr::select(genes, logFC, FDR = adj.P.Val, analysis) |>
...
cowplot::plot_grid(overall_volcano_plots, 
                   overlapping_genes_plot,
                   labels = "AUTO",
                   label_size = 10,
                   ncol = 1, 
                   rel_heights = c(2, 1))

ggsave(filename = here::here("Mariana", "Env Int Revision", "output", "figure2_PANEL.pdf"),
       width = 3, height = 9)
```
...
```{r}
write_csv(full_res, file = here::here("Mariana", "Env Int Revision", "output", "full_BW_PW_ratio_results.csv"))

write_csv(supplementary_table_overall, file = here::here("Mariana", "Env Int Revision", "output", "supplementary_table_placental_overall_S7.csv"))

write_csv(supplementary_table_sex_specific, file = here::here("Mariana", "Env Int Revision", "output", "supplementary_table_placental_sex_specific_S8.csv"))
```
...
ggsave(filename = here::here("Mariana", "Env Int Revision", "output", "placenta_weight_nonlinear_supplementary_figure.pdf"),
...
module_plot_all <- ggplot(module_dat, aes(x = analysis, y = module, fill = estimate)) +
  ...
  scale_size_area(name = "Abs(Effect Estimate)") +
  theme_bw() +
  theme(legend.position = "right",
...
Write full results for downstream analysis/figures 

```{r}
dplyr::bind_rows(
  intmod_ratio |> dplyr::mutate(analysis = "BW:PW"),
  intmod_bw |> dplyr::mutate(analysis = "BW~adj~")) |>
  write_csv(here::here("Mariana", "Env Int Revision", "output", "WGCNA_bw_ratio_fullResults.csv"))
```
