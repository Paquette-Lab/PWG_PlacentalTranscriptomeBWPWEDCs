...
## Trimester 1- Phthalates RNAseq

Running the phthalate analysis using only samples collected in the 1st trimester. 
...
load(file="/Volumes/paquette_a/GAPPSGrant_PrelimData/Sam/Data/SplitVisits/T1_FilteredNormalizedRNAseq_N49_062725.RData")

## Filtered and Natural Log Transformed Phthalate Data
PHT <-read.csv("/Volumes/paquette_a/GAPPSGrant_PrelimData/Sam/Data/SplitVisits/T1_NaturalLogPhthalates_N49_062725.csv")
rownames(PHT) <- PHT$X
PHT <- PHT %>%
  dplyr::select(-X)

## Filtered covariates
load(file="/Volumes/paquette_a/GAPPSGrant_PrelimData/Sam/Data/SplitVisits/T1_Covariates_N49_062725.RData")
covars <- covar_complete_T1
...
sum(rownames(covar_PHT) == colnames(Y_norm_T1$counts))

#Check variable types
summary(covar_PHT)

covar_PHT$race_collapsed <- factor(covar_PHT$race_collapsed, levels=c("White", "Other"))
...
all_PHT_results <- lapply(colnames(PHT)[1:16], function(phthalate) {
  ...
  ## now, limma-voom
  v <- voom(Y_norm_T1, mod, plot = TRUE)  
...
#write.csv(all_PHT_results,"/Volumes/paquette_a/GAPPSGrant_PrelimData/Sam/Data/SplitVisits/Phthalate_AllResults_T1_SplitVisits_N49_071025.csv")
```
