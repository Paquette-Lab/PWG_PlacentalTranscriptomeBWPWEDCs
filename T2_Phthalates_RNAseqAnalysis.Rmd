...
---
## Trimester 2- Phthalates RNAseq

Running the phthalate analysis using only samples collected in the 2nd trimester. 
...
load(file="/Volumes/paquette_a/GAPPSGrant_PrelimData/Sam/Data/SplitVisits/T2_FilteredNormalizedRNAseq_N139_062725.RData")

## Filtered and Natural Log Transformed Phthalate Data
PHT <-read.csv("/Volumes/paquette_a/GAPPSGrant_PrelimData/Sam/Data/SplitVisits/T2_NaturalLogPhthalates_N139_062725.csv")
rownames(PHT) <- PHT$X
PHT <- PHT %>%
  dplyr::select(-X)

## Filtered covariates
load(file="/Volumes/paquette_a/GAPPSGrant_PrelimData/Sam/Data/SplitVisits/T2_Covariates_N139_062725.RData")
covars <- covar_complete_T2
...
sum(rownames(covar_PHT) == colnames(Y_norm_T2$counts))
...
all_PHT_results <- lapply(colnames(PHT)[1:16], function(phthalate) {
  ...
  ## now, limma-voom
  v <- voom(Y_norm_T2, mod, plot = TRUE)  
...
#write.csv(all_PHT_results,"/Volumes/paquette_a/GAPPSGrant_PrelimData/Sam/Data/SplitVisits/Phthalate_AllResults_T2_SplitVisits_N139_071025.csv")
```
